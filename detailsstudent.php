...
<head>
    ...
    <title>AppliedHub | Student Details</title>
</head>
<body>
    ...
    <div class="container">
        <aside class="sidebar bg-primary">
            ...
            <div class="sidebarLinks">
                ...
                <a href="./detailsstudent.php" class="active"><i class='bx bxs-book-reader'></i> Student Details</a>
        ...
        <main class="content">
            ...
            <h2>AppliedHub | Student Details</h2>
...
<?php
            ...
            $sql_dep = "SELECT DISTINCT stdep FROM students";
            $result_dep = mysqli_query($conn, $sql_dep);

            if (mysqli_num_rows($result_dep) > 0) {
                while ($row = mysqli_fetch_assoc($result_dep)) {
                    $selected = (isset($_GET['stdep']) && $_GET['stdep'] == $row['stdep']) ? 'selected' : '';
                    echo "<option value='" . $row['stdep'] . "' $selected>" . $row['stdep'] . "</option>";
                }
            }
?>
                        </select>
...
<?php
    ...
    $sql_batch = "SELECT DISTINCT stbatch FROM students";
    $result_batch = mysqli_query($conn, $sql_batch);

    if (mysqli_num_rows($result_batch) > 0) {
        while ($row = mysqli_fetch_assoc($result_batch)) {
            $selected = (isset($_GET['stbatch']) && $_GET['stbatch'] == $row['stbatch']) ? 'selected' : '';
            echo "<option value='" . $row['stbatch'] . "' $selected>" . $row['stbatch'] . "</option>";
        }
    }
?>
                        </select>
                ...
                <a href="profilestudent.php?reg" class="bg-primary"><i class='bx bxs-user-plus' ></i> Add New Student</a>
            </div>
...
$option2 = isset($_GET['batch']) ? $_GET['batch'] : '';
...
if (!empty($option1)) {
    $conditions[] = "stdep = '$option1'";
}

if (!empty($option2)) {
    $conditions[] = "stbatch = '$option2'";
}
...
?>
                    <tr>
                        ...
                        <td><?=$row['stdep']?></td>
                        <td><?=$row['stbatch']?></td>
                        <td <?=$row['ststatus']==0?'style="color: red;"':''?>><?=$row['ststatus']==0?'Dropped Out':'Active'?></td>
                        <td><a href="profilestudent.php?reg=<?=$row['streg']?>" class="hover-bg-primary"><i class='bx bx-link-external'></i> View</a></td>
                    </tr>
<?PHP
        $count++;
    }
...
echo '<script>document.querySelector(".count").innerHTML="'.$count.' Student(s) Found"</script>';
?>
                </table>
            </div>            
        </main>
